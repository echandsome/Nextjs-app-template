...
WORKDIR /Nextjs-app-template
COPY --from=deps /Nextjs-app-template/node_modules ./node_modules
COPY . .
...
# ==========================================
RUN if test -e ./custom; then cp -avr ./custom/pages/ /Nextjs-app-template/; cp -avr ./custom/src/ /Nextjs-app-template/; cp -avr ./custom/public/ /Nextjs-app-template/; fi
...
COPY --from=builder /Nextjs-app-template/public ./public
...
COPY --from=builder --chown=nextjs:nodejs /Nextjs-app-template/.next/standalone ./
COPY --from=builder --chown=nextjs:nodejs /Nextjs-app-template/.next/static ./.next/static
...
# USER root
# RUN mkdir -p /Nextjs-app-template/backend
# RUN mkdir -p /Nextjs-app-template/backend/libs
...
# RUN mkdir -p /Nextjs-app-template/plugins
# COPY plugins/ /Nextjs-app-template/plugins

# # execute the ls command inside the image's shell to recursively list all subdirectories' content of the "WORKDIR" folder
# RUN ls -la /Nextjs-app-template/plugins
